...
(`check`/`emit`) always uses the bundled schema, which is the single
validation source.
...
analysis/review tools beyond a no-op sample, `azureDevOps` fields all `null`.
A fresh repo runs the skill's steps without touching any external system.
`additionalProperties` is `false` everywhere, so a typo'd key is a validation
...
later is a config change (a new entry), not an orchestrator change.
...
| `aggregate.kind` / `aggregate.run` | `command` / `null` | Optional report aggregator. `null` → aggregation is skipped. |
...
| `generate.kind` / `generate.run` | `command` / `null` | Optional notes/changelog generator. `null` → notes generation is skipped. |
...
| Field | Default | Meaning |
| --- | --- | --- |
| `organization` | `null` | Azure DevOps organization name. Inert when unset. Combined with `baseUrl` and `project` to build work-item ticket links and to scope `az boards` calls. |
| `project` | `null` | Azure DevOps project name. Inert when unset. Used in the work-item ticket-link path. |
| `baseUrl` | `null` | Azure DevOps base URL (e.g. `https://dev.azure.com`). Inert when unset. The link base and the `az boards --organization` value. |

All three are optional and `null` by default. When any is unset the feature is
inert: collectors keep their unconfigured record shape and `az boards` calls
use their default scope. When all three are non-empty, work-item records gain a
`url` of `<baseUrl>/<organization>/<project>/_workitems/edit/<id>` and
`az boards work-item show` is scoped with
