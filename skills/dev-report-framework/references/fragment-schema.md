...
| `category`     | enum          | yes      | One of `architecture`, `evolution`, `dependencies`, `quality`, `security`, `schema`, `contracts`, `mission`. |
...
Nine types in v1; the renderer has exactly one function per type.

| `type`         | Required shape (beyond `type`/`title`) |
| -------------- | -------------------------------------- |
| `markdown`     | `md: string` (GitHub-flavored, sanitized before injection). |
| `table`        | `columns:[{key,label,type:"string\|number",sortable?}]`, `rows:[obj]`; optional `filterable`, `defaultSort:{key,dir}`. `columns` non-empty. |
...
| `d3-graph`     | `nodes:[{id,label?,group?}]` (non-empty), `links:[{source,target,value?}]`, `layout:"force\|dag"`. |
...
forward-compatible by design — only the nine known types have their inner
shape checked.
