...
| [dev-report-framework](dev-report-framework/SKILL.md) | Aggregate report fragments from the dev-analysis/dev-test Skills into one standalone HTML report folder per release, with a this-vs-previous split-screen. Owns the fragment JSON contract, `validate_fragments.py`, and `dev-report-build`. |
| [dev-analysis-evolution](dev-analysis-evolution/SKILL.md) | Analyze git history across releases — code survival (git-of-theseus), churn/coupling/authorship (code-maat via Docker), per-extension/per-author change distribution, and per-author PR activity classification with an optional "vibe coder" label. Emits `evolution` and `author-activity` fragments. |
...
| [dev-analysis-architecture](dev-analysis-architecture/SKILL.md) | Extract structure and check it against intent — dependency-cruiser, Nx Graph, madge, Structurizr, MSADoc, ArchUnitNET/NetArchTest, cargo-modules. Emits an `architecture` fragment with a dependency graph and cycle/depth metrics. |
| [dev-analysis-dependencies](dev-analysis-dependencies/SKILL.md) | Scan third-party dependencies and supply chain — OWASP Dependency-Check, Trivy, Syft+Grype, cargo-audit/cargo-geiger — deduped across scanners. Any critical vulnerability ⇒ status error. |
