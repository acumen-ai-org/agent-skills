...
| [dev-report-framework](dev-report-framework/SKILL.md) | Aggregate report fragments into one standalone HTML "Release candidate report" per release: every fragment in a two-column This-release / vs-production layout, producer-declared per-part top menu, in-place file/markdown preview, show/hide previous releases, a global `Module:` filter (from `dev-process.json` `modules`), optional DESIGN.md theming. Owns the fragment JSON contract (`view` tag, `files[]`, table `children`, `image`, `module`/`type:"module"`, `d3-graph` force/dag/chord), `validate_fragments.py`, and `dev-report-build`. |
